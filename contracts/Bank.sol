...
contract Bank is Ownable {
     ...
     */
    function withdraw(string memory _symbol, uint256 _amount)
        external
        onlyOwner
    {
        ...
        if (_amount > 0) {
            ...
            tokenOwnerBalance[_tokenAddress][owner] = SafeMath.sub(
                tokenOwnerBalance[_tokenAddress][owner],
                _amount
            );
        }
        require(
            token.transferFrom(address(this), owner, _amount) == true,
     ...
     */
    function deposit(string memory _symbol, uint256 _amount) external payable {
        address _tokenAddress = tokensAllowed[_symbol];
        ...
        tokenOwnerBalance[_tokenAddress][owner] = SafeMath.add(
            tokenOwnerBalance[_tokenAddress][owner],
            _amount
        );

        require(
            token.transferFrom(owner, address(this), _amount) == true,
            "Transfer not complete"
        );
     ...
     */
    function addToken(string memory _symbol, address _tokenAddress)
        external
        onlyOwner
        returns (bool)
    {
     ...
     */
    function removeToken(string memory _symbol)
        external
        onlyOwner
        returns (bool)
    {
        require(tokensAllowed[_symbol] != address(0x0));
     ...
     */
    function totalTokenSupply(string memory _symbol)
        public
        view
        returns (uint256)
    {
     ...
     * for a certain token
     */
    function balanceOf(string memory _symbol, address _customerAddress)
        public
        view
        returns (uint256)
    {
