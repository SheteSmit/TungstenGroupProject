...
contract EscrowManager is Ownable {
    ...
    */
    function createItemAndDepositFunds(address payable _payee, string memory _escrowDetails, uint _priceInWei)
    external
    payable
    {
        ...
        escrows[index].paidWei = SafeMath.sub(msg.value, SafeMath.findFee(msg.value));
        ...
        emit JobState(index, uint(escrows[index].state));
        index++;
    }
    ...
    */
    function jobCompletion(uint _index, bool _completed) public {
        require(escrows[_index].payee == msg.sender, "Only the payee is allowed to complete job.");
        require(escrows[_index].state == State.ESCROW_CREATED_PAYMENT_DEPOSITED, "Job is not at this state yet.");

        escrows[_index].jobCompletion = _completed;

        if (_completed) {
            escrows[_index].state = State.JOB_COMPLETED;
            emit JobState(_index, uint(escrows[_index].state));
        } else {
            escrows[_index].state = State.DISPUTE;
            emit JobDispute(_index, uint(escrows[_index].state));
        }
    ...
    */
    function confirmJobCompletion(uint _index, bool _satisfied) external {
        require(escrows[_index].escrowCreator == msg.sender, "Only the job creator can confirm job completion.");
        require(escrows[_index].state == State.JOB_COMPLETED, "Job is not at this state yet.");
    ...
    */
    function resolveDispute(uint _index, State _setState) external onlyOwner {
        require(escrows[_index].state == State.DISPUTE, "This job is not in dispute.");
    ...
    */
    function sendFundsToCreator(uint _index) private {
        require(ETH_ADDRESS.universalTransfer(escrows[_index].escrowCreator, escrows[_index].paidWei), "Transfer didn't complete");
    ...
    */
    function sendFundsToPayee(uint _index) private {
        require(ETH_ADDRESS.universalTransfer(escrows[_index].payee, escrows[_index].paidWei), "Transfer didn't complete.");
        transactions[_index][escrows[_index].escrowCreator] = 0;
